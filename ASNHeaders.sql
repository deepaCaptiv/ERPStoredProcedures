...
BEGIN    
  ...
  FROM @tempasnheader  
   ...
   IF EXISTS (SELECT 1 FROM vertiv..invoiceasndetails WITH(NOLOCK) WHERE ASNId=@AsnId )  
   BEGIN  
    
  INSERT INTO InvoiceASNDetailsHistory  
  (  
   InvoiceASNDetailsHistoryId,RecordedOn,ASNId,ASNNubmer,ShippingDate,ExpectedRecieptDate,ModeOfTransport,Remarks,  
   VehicleNo,DriverName,DriverLicenseNo,DriverPhone,LRNo,LRDate,TransportCompanyName,TransmitTime,RRNo,RRDate,  
   AirwayBillNo,FlightNo,FlightDate,AirlineNo,BillofLadingNo,BOLDate,VesselName,voyageNo,CourierNo,TrackingNo,  
   DockerNo,InvoiceId,VendorName,ShipToLocation,NumOfContainers,FreightCarrier,WaybillNumber,PackingSlip,DocId,  
   AsnStatus,IsERP  
  )  
  SELECT   
   NEWID(),GETDATE(),ASNId,ASNNubmer,ShippingDate,ExpectedRecieptDate,ModeOfTransport,Remarks,  
   VehicleNo,DriverName,DriverLicenseNo,DriverPhone,LRNo,LRDate,TransportCompanyName,TransmitTime,RRNo,RRDate,  
   AirwayBillNo,FlightNo,FlightDate,AirlineNo,BillofLadingNo,BOLDate,VesselName,voyageNo,CourierNo,TrackingNo,  
   DockerNo,InvoiceId,VendorName,ShipToLocation,NumOfContainers,FreightCarrier,WaybillNumber,PackingSlip,DocId,  
   AsnStatus,IsERP  
  FROM vertiv..invoiceasndetails WITH(NOLOCK)   
  WHERE ASNId=@AsnId   
   ...
   ---t.BOLDate,    
        ...
        LEFT JOIN vertiv..invoiceasndetails p  WITH(NOLOCK)  
            ON t.ASNId = p.ASNId    
