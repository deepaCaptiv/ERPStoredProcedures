select  docid* from PaymentReceipt

select  docid,* from PaymentInvoiceMapping


  sp_helptext UpdateASNDocId


   

    select *from InvoiceASNDetails
...
BEGIN    
   ...
   tar.last_update_login=src.last_update_login    
        ...
        INNER JOIN @tempheader src    
            ON tar.payment_id = src.record_id;    
   ...
   last_update_login    
        FROM @tempheader src    
        WHERE NOT EXISTS (    
            SELECT 1 FROM PaymentReceipt p WHERE p.payment_id = src.record_id    
        );    
        ...
        FROM @tempheader src    
        WHERE NOT EXISTS (    
            SELECT 1 FROM PaymentInvoiceMapping p WHERE p.payment_id = src.record_id    
        );    
        ...
        FROM @tempheader src    
        WHERE NOT EXISTS (    
            SELECT 1 FROM PaymentPOHeaderMapping p WHERE p.Payment_Id = src.record_id    
        );    
     ...
     -------------------------------------------------------------    
        ...
        FROM @tempheader src    
        WHERE NOT EXISTS (    
      SELECT 1 FROM PaymentASNMapping p WHERE p.Payment_Id = src.record_id    
        );    
