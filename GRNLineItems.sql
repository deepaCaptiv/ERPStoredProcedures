...
BEGIN            
 ...
 FROM @templineitems  
  ...
  IF EXISTS ( SELECT 1 FROM vertiv..grnlineitems WITH(NOLOCK) WHERE grn_id=@GrnID)  
  BEGIN  
  
  
    INSERT INTO GRNLineItemsHistory  
 (  
  GRNLineItemsHistoryId,RecordedOn,grn_line_id,grn_id,line_num,po_line_num,po_shipment_num,  
  item_code,item_desc,qty_ordered,qty_received,qty_accepted,qty_rejected,uom,unit_price,  
  line_amount,destination_type,subinventory,locator_id,inspection_status,created_at,  
  updated_at,deleted_at  
 )  
 SELECT NEWID(),GETDATE(),grn_line_id,grn_id,line_num,po_line_num,po_shipment_num,  
   item_code,item_desc,qty_ordered,qty_received,qty_accepted,qty_rejected,uom,unit_price,  
   line_amount,destination_type,subinventory,locator_id,inspection_status,created_at,  
   updated_at,deleted_at   
 FROM vertiv..grnlineitems PLI WITH(NOLOCK)  
 WHERE grn_id=@GrnID  
 ...
 --Insert new Line Items          
  ...
  SELECT        
      ...
      ISNULL(SRC.created_at, GETDATE()),      
               ...
               SRC.deleted_at       
  FROM @TempLineItems SRC   
  LEFT JOIN Vertiv..GRNLineItems PLI      
  ON SRC.grn_line_id = PLI.grn_line_id   
  WHERE PLI.grn_line_id IS NULL;        
    
  DELETE PLI  
  FROM vertiv..grnlineitems PLI  
  WHERE grn_id=@GrnID AND   
  PLI.grn_line_id NOT IN (   
                         SELECT grn_line_id   
          FROM @TempLineItems   
          )  
